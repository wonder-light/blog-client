<template>
  <div class="editor">
    <div v-if="type==='comment'" class="form-info">
    ...
    <div v-if="type==='comment'" class="action-bottom">
...
let props = defineProps({
    type: {type: String, default: 'normal' | 'comment' | 'readonly'},
...
function created() {
    ...
    if (id.trim().length > 0 && isNaN(Number(id))) {
        ...
        editorId.value = proxy.functions.NewEditorId();
    }
    userCopy = CopyUser();
}
...
function Submit() {
    ...
    form.value.validate(async (isValid, invalidFields) => {
        ...
        if (isValid === false) {
            ...
            let message = '';
            if (user.id <= 0) {
                await proxy.axios.post('/user/match', user).then(response => {
                ...
                if (proxy.functions.ObjectEqual(user, userCopy)) {
                    state = true;
                }
                else {
                    await proxy.axios.post(`/user/tourist/${user.id}`, user).then(() => {
                        state = true;
                        userCopy = CopyUser();
                    }).catch(() => {
                        state = false;
